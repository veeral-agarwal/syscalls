...
int main(int argc, char *argv[])
{
    ...
    if (argc != 4)
    {
        print("Error: Invalid arguments given. Please provide file name and two single digit numbers.\n");
        return 1;
    }
    ...
    strcat(destination, "./Assignment/2_");
    ...
    lli s = lseek(source_path, 0, SEEK_END);
    lli total_size = s;
    lli chunk_count = argv[2][0] - '0' + 0ll;
    lli chunk_nubmer = argv[3][0] - '0' + 0ll;
    lli siz = total_size / chunk_count;

    if (chunk_count == 0 || total_size % chunk_count != 0)
    {
        print("The file cannot be split into equal chunks\n");
        return 1;
    }

    if (chunk_nubmer > chunk_count)
    {
        print("Invalid chunk number");
        return 1;
    }

    lli start_offset = siz * (chunk_nubmer);
    lli offset = lseek(source_path, start_offset, SEEK_SET);
    lseek(write_file, 0, SEEK_SET);

    lli progress = 0;
    char progress_string[100];
    lli end_offset = siz * (chunk_nubmer - 1);

    while (offset > end_offset)
    {
        float val = 100.0 * progress / siz;
        sprintf(progress_string, "\r%.2f%% file copied", val);
        write(1, progress_string, strlen(progress_string));

        lli length_of_data;
        if (offset - end_offset >= BUFFER_SIZE)
            length_of_data = BUFFER_SIZE;
        else
            length_of_data = offset - end_offset;
